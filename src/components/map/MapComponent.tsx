...
import { MapMandi, OptimalRoute, LocationPoint } from '../../services/map/mapApiService';
...
const createMandiIcon = (type: string, isSelected: boolean = false) => {
...
const MapComponent: React.FC<MapComponentProps> = ({ 
  ...
  const center: [number, number] = userLocation ? [userLocation.lat, userLocation.lon] : defaultCenter;
  ...
  useEffect(() => {
    if (map && mandis.length > 0) {
      const markers = mandis.map(mandi => 
  ...
  const isMandiSelected = (mandi: MapMandi): boolean => {
  ...
  const renderRoutePolyline = () => {
    ...
    return optimalRoute.routePlan.detailedRoutes.map((segment, index) => {
      if (segment.route && segment.route.routePoints) {
        const positions: [number, number][] = segment.route.routePoints.map(point => [
          point.lat, 
          point.lon
        ]);
  ...
  const MarkerPopup: React.FC<{ mandi: MapMandi }> = ({ mandi }) => {
        ...
        <div className="min-w-64 p-2">
          <div className="flex items-start justify-between mb-2">
            ...
            <div className="flex items-center space-x-1">
              <span 
                className="px-2 py-1 text-xs rounded-full rating-badge"
                data-rating={mandi.rating}
              >
                ⭐ {mandi.rating}
              </span>
              {mandi.isVerified && (
